...
public class ProjectServiceImpl implements ProjectService {
    ...
    public List<EmployeeResponse> getMatchingEmployees(Long projectId, Long requirementId) {
        ...
        return employeeRepository.findAll().stream()
                ...
                .filter(emp -> emp.getExperienceYears() >= minExp)
                .filter(emp -> {
                    Set<String> empSkills = Arrays.stream(emp.getSkills().split(","))
                            .map(String::trim)
                            .map(String::toLowerCase)
                            .collect(Collectors.toSet());
    ...
    private EmployeeResponse toEmployeeResponse(Employee e) {
        return new EmployeeResponse(e.getId(), e.getName(), e.getEmail(),
                e.getRole(), e.getSkills(), e.getExperienceYears());
    }
}